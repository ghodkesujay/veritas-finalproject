 ...
 * @author sujayghodke
 */
public class PatientMenuPanel extends javax.swing.JPanel {
    ...
    EcoSystem system;
    Patient patient;
    ArrayList<HBloodRequirement> items=new ArrayList<HBloodRequirement>();
    /**
     * Creates new form RequestLabTestJPanel
     */
    public PatientMenuPanel(JPanel userProcessContainer, UserAccount account,EcoSystem system,Hospital hospital) {
        initComponents();
     ...
     public void populateTable(){
            ...
            model.setRowCount(0);
                ...
                for(HBloodRequirement dish:hospital.getMenu()){
                     row[0] = dish;
     ...
     public void populateCart(HBloodRequirement item){
        DefaultTableModel model = (DefaultTableModel) cartTable.getModel();
        model.setRowCount(0);
        
         items.add(item);
         Object[] row = new Object[3];
                for(HBloodRequirement dish:items){
                     row[0] = dish;
                     row[1] = dish.getDescription();
                     row[2] = dish.getPrice();
    ...
    private void initComponents() {
        ...
        add(cardBtn, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 240, -1, -1));
    ...
    private void cardBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cardBtnActionPerformed
        ...
        if(selectedRow<0){
            JOptionPane.showMessageDialog(null,"Please select a row from the table to receive donation","Warning",JOptionPane.WARNING_MESSAGE);
        }
        else{
            HBloodRequirement item=(HBloodRequirement)menuTable.getValueAt(selectedRow, 0);
    ...
    private void orderBtn1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_orderBtn1ActionPerformed
        ...
        hospital.addOrder(hospital.getName(), userAccount.getUsername(), null, items, String.valueOf(sum) , address);
        for(Patient cust:system.getPatientDirectory().getCustList()){
            if(userAccount.getUsername().equals(cust.getUserName())){
                //cust.addOrder(patient.getName(),items,hospital.getName(), address );
                cust.addOrder(hospital.getName(), userAccount.getUsername(), null, items, String.valueOf(sum) , address);
            }
        }
        
        
        JOptionPane.showMessageDialog(null,"Your request to receive blood is placed","Thank You",JOptionPane.WARNING_MESSAGE);
        sum=0;
        userProcessContainer.remove(this);
        Component[] componentArray = userProcessContainer.getComponents();
        Component component = componentArray[componentArray.length - 1];
    ...
    private void RemoveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RemoveBtnActionPerformed
        ...
        if(selectedRow<0){
            ...
            HBloodRequirement item=(HBloodRequirement)cartTable.getValueAt(selectedRow, 0);
